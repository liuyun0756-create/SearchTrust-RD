...
CMD ["/bin/sh", "-c", "if [ \"${SEARCHTRUST_SERVICE_ROLE:-web}\" = \"worker\" ]; then exec arq app.jobs_v22.worker.WorkerSettings; else exec uvicorn app.main:app --host 0.0.0.0 --port ${PORT:-8000} --workers 1 --loop uvloop --http httptools; fi"]
