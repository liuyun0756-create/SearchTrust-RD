...
from app.report_v22.public_rule_catalog import GBP_RULE_SPECS, RULES, RULE_VERSION
...
def decision(view, rule_id, target, state, reason, *, evidence=(), comparators=(),
             statement=None, notes=(), severity=None, rule_version=RULE_VERSION):
    evidence = sorted(set(evidence))
    comparators = sorted(set(comparators))
    limitations = sorted(set([SAMPLE_NOTE, *notes, *view.notes(evidence + comparators)]))
    finding = None
    if state == "triggered":
        specs = {**RULES, **GBP_RULE_SPECS}
        if not evidence or not statement or rule_id not in specs:
