...
class FindingsError(DeterministicJobError):
    def __init__(self, kind: Literal[
        "INPUT_INVALID", "REFERENCE_INVALID", "ID_CONFLICT", "LIMIT_EXCEEDED",
        "BINDING_INVALID", "CHECKSUM_MISMATCH",
    ]):
        super().__init__(f"V22_FINDINGS_{kind}", {
            ...
            "BINDING_INVALID": "Public findings sources do not match the confirmed business identity.",
            "CHECKSUM_MISMATCH": "Public findings source content does not match its bound checksum.",
        }[kind])
