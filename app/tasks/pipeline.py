...
async def _run_pipeline(
    ...
    Parameters
    ----------
    created_at: Task creation timestamp (ISO 8601). Passed in from the API
                layer so the exact same value is preserved across all state
                updates, including failure paths that live outside this
                function's scope.
...
async def analyze_pipeline(
    ...
    created_at: Task creation timestamp (ISO 8601), generated by the API layer
                and passed through so all state updates — including failure
                paths — record the original creation time, not the current time.

    Returns
    -------
    dict — final analysis report (also stored in Redis).
    """
    # Fallback guards against direct task invocation that omits created_at
    # (e.g. manual celery shell calls during development).
    if not created_at:
