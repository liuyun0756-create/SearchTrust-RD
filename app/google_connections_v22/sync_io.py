...
class TokenBroker:
    def __init__(self, origin: str, secret: str, client: httpx.AsyncClient):
        ...
        self.origin, self.secret, self.client = origin.rstrip("/"), secret, client

    async def access_token(self, connection_id: str) -> str:
        connection_id = str(UUID(connection_id))
        body = json.dumps({"connection_id": connection_id, "purpose": "source_sync", "source": "gsc"}, separators=(",", ":"))
        ...
        try:
            ...
            if not isinstance(scopes, list) or not all(isinstance(scope, str) for scope in scopes) or GSC_SCOPE not in scopes:
                raise ValueError()
...
class SyncRepository:
    def __init__(self, url: str, key: str, client: httpx.AsyncClient):
        ...
        self.url, self.key, self.client = url.rstrip("/"), key, client
    ...
    async def pending(self):
        ...
        data = await self.request("GET", "google_sync_jobs", params={"select": "id", "limit": "20", "order": "created_at.asc",
    ...
    async def claim(self, job_id: str):
        data = await self.request("POST", "rpc/claim_v22_gsc_sync", body={"p_job_id": job_id})
    ...
    async def finish(self, job: dict, payload: dict, checksum: str, health: str, reasons: list[str]):
        return await self.request("POST", "rpc/finish_v22_gsc_sync", body={"p_job_id": job["id"], "p_lease_id": job["lease_id"],
            "p_payload": payload, "p_checksum": checksum, "p_health": health, "p_reasons": reasons})

    async def fail(self, job: dict, error: SyncError):
        await self.request("POST", "rpc/fail_v22_gsc_sync", body={"p_job_id": job["id"], "p_lease_id": job["lease_id"],
