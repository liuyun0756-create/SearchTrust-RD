...
> **数据库前置条件：** 后端部署不执行 Supabase migration。凡是前端报告字段发生变化，必须由项目负责人先在 Supabase SQL Editor 手动执行对应 SQL，并确认字段存在。迁移 SQL 未执行时，部署不能作为完整上线验收。详细规则见 [跨系统变更与数据库交接规范](docs/CHANGE_MANAGEMENT.md)。
...
DIFY_STREAM_TIMEOUT=1200
DIFY_RETRY=3
TASK_STREAM_TIMEOUT=1260
TASK_STREAM_HEARTBEAT_INTERVAL=20

# SerpAPI
SERPAPI_KEY=xxx
SERPAPI_KEY_SECONDARY=xxx
SERPAPI_KEY_TERTIARY=xxx
...
| 代码 | GitHub | ✅ 新平台连接同一仓库 |
| Supabase 表、字段、索引、RLS | Supabase 项目 | ✅ **项目负责人在 SQL Editor 手动执行迁移并确认** |

当前 Native v2.1 报告要求 `reports.report_v2_1 jsonb null`。应用代码对缺失列有旧字段回退，但该回退不保存 v2.1 JSON，不能视为数据库迁移完成。
